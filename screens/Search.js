    ...
    render(){
        return (
            <View>
                ...
                <FlatList data={this.state.users} renderItem ={({item}) => (<UserCard click = {() => this.props.navigation.navigate("Profile", {name: item.name, username: item.username, id: item.id})} navigate = {this.props.navigation} name = {item.name} username = {item.username} id = {item.id} />)} keyExtractor ={item => item.id} />
            </View>
        )
    }
